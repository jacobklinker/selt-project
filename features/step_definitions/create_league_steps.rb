When /^I create a league with "(.*?)" as a Leage Name and "(.*?)" as conference settings and "(.*?)" as number of picks and email list containing "(.*?)"$/ do |league_name,conf_settings,num_picks,emails|
